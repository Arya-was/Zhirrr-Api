...
loghandler = {
    ...
    invalidKey: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'apikey invalid, gak punya apikey? chat gw aja yaaa di wa.me/62895331406727 , key nya gratis kok gan, jadi santuyy ajaa'
    },
    invalidlink: {
        status: false,
        creator: `${creator}`,
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------Loli'+'Anu';
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'anu') return res.json(loghandler.invalidKey)
...
router.get('/textmaker/senja', async (req, res, next) => {
	...
	if(apikeyInput != 'anu') return res.json(loghandler.invalidKey)
        if (!theme) return res.json(loghandler.nottheme)
        if (theme != 'coffee-cup' && theme != 'coffee-cup2' && theme != 'gradient') return res.json(loghandler.notheme)
        if (!text) return res.json(loghandler.nottext)
